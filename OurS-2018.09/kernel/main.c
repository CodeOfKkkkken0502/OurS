   ...
   if(strcmp(tty_name, "/dev_tty0")==0){
    ...
    while (1) {  
        ...
        else if (strcmp(cmd, "test") == 0)
        {
            ...
            if(fd == -1){
                printf("Path doesn't exist. Please check the pathname.\n");
            }
            else{
...
void doEncrypt(char *path, int fd_stdin)
{
    ...
    printl("Please enter the new file password: ");
    read(fd_stdin, pass, 128);
...
void help()
{
    ...
    printf("5.  new         [file]            : Create a new file\n");
