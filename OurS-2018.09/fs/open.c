...
PRIVATE struct inode * new_inode(int dev, int inode_nr, int start_sect, int inode_type, int imode);
PRIVATE void new_dir_entry(struct inode * dir_inode, int inode_nr, char * filename, int type, char* pth);
...
for(int i=0;i<10000;i++){for(int j=0;j<10000;j++){}}*/
	...
	if (flags & O_CREAT) {
		...
		pin = get_inode(dir_inode->i_dev, inode_nr);
                //printl("\n%d     %d\n",dir_inode->i_dev, inode_nr);
                if(get_inode(dir_inode->i_dev, inode_nr)){printl("\nRIGHT.\n");}
	}
...
PRIVATE struct inode * create_file(char * path, int flags)
{
	...
	struct inode *newino = new_inode(dir_inode->i_dev, inode_nr,
		free_sect_nr, inode_type, I_REGULAR);
	new_dir_entry(dir_inode, newino->i_num, filename, inode_type, path);
	return newino;
}
...
PRIVATE struct inode * create_dir(char * path)
{
	...
	int free_sect_nr = alloc_smap_bit(dir_inode->i_dev,
		NR_DEFAULT_FILE_SECTS);
	struct inode *newino = new_inode(dir_inode->i_dev, inode_nr,
		free_sect_nr, inode_type, I_DIRECTORY);
...
PUBLIC int do_ls()
{
	...
	for (i = 0; i < nr_dir_blks; i++)
	{
		...
		for (j = 0; j < SECTOR_SIZE / DIR_ENTRY_SIZE; j++, pde++)
		{
			...
			char type[10];
			if (pde->type == 1) { strcpy(type, "File"); }
			else { strcpy(type, "Directory"); }
			printl("  %2d       %s         %s\n", pde->inode_nr, pde->name, type);
			if (++m >= nr_dir_entries) {
				printl("\n");
				break;
			}
...
*****************************************************************************/
PRIVATE void new_dir_entry(struct inode *dir_inode, int inode_nr, char *filename, int type, char* path)
{
	/* write the dir_entry */
	int dir_blk0_nr = dir_inode->i_start_sect;
	...
	int j = 0;
	i++;
	while (i < strlen(pathname)) { parname[j] = pathname[i]; i++; j++; }*/

	struct inode * parnode = search_file(path);
	...
	for (i = 0; i < nr_dir_blks; i++) {
		...
		for (j = 0; j < SECTOR_SIZE / DIR_ENTRY_SIZE; j++, pde++) {
			...
			if (pde->inode_nr == parnode->i_num) { /* it's a free slot */
				pardir = pde;
				break;
			}
		}
